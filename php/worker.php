      ...
      <div class="row">
        <div  class="col-sm-3 col-md-2 sidebar" >
          <ul class="nav nav-sidebar">
            <li class="active"><a href="#">ALL SHOP WORKS</a></li>
            <li ><a href="d_works.php">CUSTOMER WORKS</a></li>
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main" style="background-image:url('images/1.jpg') ;background-repeat: no-repeat; background-position: center;background-size: cover;">
          ...
          <?Php
					...
					$q="SELECT * FROM `shops` join `works` on `shops`.id=`works`.shop_id where `works`.status='pending';";
            ...
            <table class='table table-striped'>
              ...
              <tbody>";
					while($row=mysqli_fetch_array($res))
					{
						...
						$amnt=$row['amount'];
						$status=$row['status'];
						echo " <form class='form-signin' role='form' action='work_submit.php' method='post'><tr>
                  <td>INF$id</td>
				  <td>$type</td>
                  <td>$model</td>
                  <td>$des</td>
...
<center><h2 class="page-header">Completed Works</h1></center>
          <?Php
		            ...
		            $con=mysqli_connect("localhost","infinity","949510","infinity");
					$q="SELECT * FROM `shops` join `works` on `shops`.id=`works`.shop_id where `works`.status='Completed';";
					$res=mysqli_query($con,$q);
					$i=0;
					echo "
          <div class='table-responsive'>
        ...
        <input type='text' class='form-control' placeholder='$amnt' name='amount' value='$amnt' ><br>
         <input type='hidden' name=id value='$id' ><br>
                  ...
                  <td>    <select class='form-control' class='form-control' name='status' required > <option value='$status'>$status
    </option  >";
...
?>
          ...
          <?Php
		            ...
		            $con=mysqli_connect("localhost","infinity","949510","infinity");
					$q="SELECT * FROM `shops` join `works` on `shops`.id=`works`.shop_id where `works`.status='Cannot Repaired';";
					$res=mysqli_query($con,$q);
					$i=0;
					echo "
          <div class='table-responsive'>
